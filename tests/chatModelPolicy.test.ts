...
describe("shared chat model selection policy", () => {
  ...
  it.each(["loading", "error"] as const)("retains known restrictions during %s and does not offer unverified custom entries", (status) => {
    const policy = getChatModelPolicy(connector, { ...catalog, status });
  ...
  it.each([
    { ...connector, id: "apimart-2" },
    { ...connector, baseUrl: "https://other.example/v1" },
    { ...connector, apiKey: "rotated-key" },
  ])("does not reuse metadata after connector/credentials change: %j", (other) => {
    const policy = getChatModelPolicy(other, catalog);
  ...
  it("starts APIMart discovery unverified without presenting saved/manual choices as compatible", () => {
    const policy = getChatModelPolicy(connector, undefined);
