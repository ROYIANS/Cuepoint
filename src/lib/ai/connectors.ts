...
import { chatModelIssue, type ChatModelConnector } from "@/lib/ai/chatModelPolicy";
import {
  listModels,
  testConnection,
...
export async function runWithCompatibleChatModel<T>(
  ...
  if (connector.definitionId === "apimart") {
    ...
    if (stale()) return { ok: false, message: "模型或连接已变更，请重新发送", aborted: true };
    if (!result.ok) return { ok: false, message: `无法确认 APIMart 模型类型：${result.message}。请重试。` };
...
/** Provider-specific discovery; unknown APIMart categories are manual-entry only in chat. */
...
/** APIMart probes are read-only even for keys restricted to media models. */
