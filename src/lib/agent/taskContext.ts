...
export async function getTaskContext(threadId: string | undefined, instructions: string, taskMode = false, interactionMode: AgentInteractionMode = "smart", projectId?: string) {
  ...
  const project = boundProjectId ? await db.projects.get(boundProjectId) : undefined;
  const projectKind = project ? getProjectKind(project) : undefined;
  ...
  return { instructions: assembled, task, records, taskToolNames, projectContext, projectKind };
}
