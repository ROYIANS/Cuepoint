// Grainient shader and component adapted from the React Bits source supplied for this page.
import { useEffect, useRef } from 'react';
import { Renderer, Program, Mesh, Triangle } from 'ogl';
import './homeGrainient.css';
...
const Grainient = ({
  ...
  useEffect(() => {
    ...
    // Leave the CSS gradient visible if WebGL2 is unavailable.
    ...
    const tryStart = () => {
      if (isVisible && isPageVisible && !reducedMotion.matches && raf === 0) raf = requestAnimationFrame(loop);
    };
    const tryStop = () => {
    ...
    const io = new IntersectionObserver(
      ([entry]) => { isVisible = entry?.isIntersecting ?? false; isVisible ? tryStart() : tryStop(); },
      { threshold: 0 }
    );
  ...
  return <div ref={containerRef} aria-hidden="true" className={`home-grainient ${className}`.trim()} />;
};
