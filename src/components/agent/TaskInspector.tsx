...
import { Archive, ArrowLeft, Check, CheckCheck, ChevronDown, Circle, ListTodo, Pencil, Pin, Plus, RotateCcw, Sparkles, X } from "lucide-react";
...
function TaskInspectorContent({ task, runs, messages, open, onOpenChange, onOpenBoard }: InspectorProps) {
  ...
  const editable = !busy && task.lifecycle === "open" && !pending;
  const completed = task.plan.filter((step) => step.status === "completed").length;
  const canComplete = editable && completed === task.plan.length;
  ...
  return <Sheet open={open} onOpenChange={(next) => { if (!lock.current && !editor) onOpenChange(next); }}>
    <SheetContent className="agent-task-inspector" showCloseButton={false}>
      <div className="agent-task-inspector-nav"><Button variant="ghost" size="sm" disabled={pending} onClick={() => { onOpenChange(false); onOpenBoard(); }}><ArrowLeft />任务工作台</Button><Button variant="ghost" size="icon" aria-label="关闭任务详情" disabled={pending} onClick={() => onOpenChange(false)}><X /></Button></div>
