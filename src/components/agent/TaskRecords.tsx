...
export function TaskRecords({ task, messages, editable }: Props) {
  ...
  const evidence = [
    ...messages.filter((message) => message.threadId === task.threadId && message.role === "user" && message.content.trim()).map((message) => ({ type: "message" as const, id: message.id, label: `用户 · ${message.content.slice(0, 70)}`, body: message.content })),
