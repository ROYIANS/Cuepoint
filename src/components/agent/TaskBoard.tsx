...
export function TaskBoard({ tasks, runs, projects, onOpenTask, onBack }: {
  tasks: AgentTask[] | undefined; runs: AgentRun[] | undefined; projects: Project[];
  ...
  const scoped = entries.filter(({ task }) => !filterProjectId || task.projectId === filterProjectId);
  ...
  const completeCount = scoped.filter(({ state }) => state === "completed").length;
  async function create() {
    if (saveLock.current || !title.trim() || !goal.trim() || !projectId) { if (!projectId) toast.error("请先选择项目"); return; }
    saveLock.current = true; setSaving(true);
    try {
      const task = await createAgentTask({ projectId, title: title.trim(), goal: goal.trim() });
  ...
  return <section className="agent-task-board" aria-label="任务工作区">
    <header className="agent-task-board-heading">
      ...
      <Button onClick={() => { setProjectId(filterProjectId); setCreating(true); }} className="agent-task-create"><Plus />新建任务</Button>
    </header>
    <div className="agent-task-board-toolbar">
      <ProjectPicker projects={projects} projectId={filterProjectId} onChange={setFilterProjectId} allLabel="全部项目" />
      ...
      <span className="agent-task-board-count">{ready ? `${scoped.length} 个任务 · ${completeCount} 个已完成` : "读取工作区…"}</span>
    ...
    {!ready ? <div className="agent-task-loading" role="status">正在读取任务与执行进度…</div> : entries.length === 0 ?
      <div className="agent-task-empty"><ListTodo size={32} strokeWidth={1.25} aria-hidden /><h2>先定一个小目标</h2><p>整理角色设定、打磨故事，或准备下一次创作。<br />你可以先写下任务，随时再与助手一起推进。</p><Button onClick={() => { setProjectId(filterProjectId); setCreating(true); }}><Plus />创建第一个任务</Button><span>无需配置模型，也可以手动管理任务</span></div> :
          ...
          return <section className="agent-task-column" key={column.id} aria-label={column.label}>
            ...
            {cards.length === 0 ? <div className="agent-task-column-empty">{query || filter === "archived" ? "没有匹配的任务" : "暂无任务"}</div> : cards.map(({ task, state }) => {
              ...
              return <button className="agent-task-card" key={task.id} type="button" onClick={() => onOpenTask(task)}>
                ...
                <h3>{task.title}</h3><p>{task.goal}</p><small className="agent-task-card-project">{projects.find((project) => project.id === task.projectId)?.name ?? "项目不可用"}</small>
    ...
    <Dialog open={creating} onOpenChange={(next) => { if (!saveLock.current) setCreating(next); }}>
      <DialogContent className="agent-task-dialog" showCloseButton={!saving}>
        ...
        <form onSubmit={(event) => { event.preventDefault(); void create(); }}>
          <div className="agent-task-field">所属项目<ProjectPicker projects={projects} projectId={projectId} required allowClear={false} locked={saving} onChange={setProjectId} /></div><label className="agent-task-field">任务名称<Input autoFocus value={title} maxLength={120} onChange={(event) => setTitle(event.target.value)} placeholder="例如：完善主角的人物设定" disabled={saving} required /></label>
          ...
          <DialogFooter><Button type="button" variant="ghost" disabled={saving} onClick={() => setCreating(false)}>取消</Button><Button type="submit" disabled={saving || !projectId || !title.trim() || !goal.trim()}>{saving ? "创建中…" : "创建任务"}</Button></DialogFooter>
        </form>
      </DialogContent>
    </Dialog>
  </section>;
