...
import { addAudioSegment, deleteAudioSegment, patchAudioSegment } from "@/db/audio";
...
export function ScriptDocument({ chapter, snapshot, selectedId, busy, action, onSelect, onInspect, onVoices }: {
  chapter: AudioChapter; snapshot: AudioProjectSnapshot; selectedId: string; busy: boolean; action: AudioAction;
  onSelect: (segment: AudioSegment) => void; onInspect: (segment: AudioSegment) => void; onVoices: (segment: AudioSegment) => void;
}) {
  ...
  return <div className="as-document">
    {!segments.length ? <div className="as-blank-document"><p>从你的第一段台词开始</p><span>写下脚本，或者导入、录制已有声音。制作方式由你决定。</span><div><Button disabled={busy} onClick={() => void action(() => add())}><Plus />写下第一段</Button><Button variant="ghost" onClick={() => setPasting(true)}>粘贴完整稿件</Button></div></div> : <div className="as-paragraphs">{segments.map((segment, index) => <ScriptParagraph key={segment.id} segment={segment} index={index} siblings={segments} snapshot={snapshot} selected={selectedId === segment.id} busy={busy} action={action} onSelect={() => onSelect(segment)} onInspect={() => onInspect(segment)} onVoices={() => onVoices(segment)} onFocusNew={(id, atEnd) => setFocusRequest({ id, atEnd })} />)}</div>}
...
function ScriptParagraph({ segment, index, siblings, snapshot, selected, busy, action, onSelect, onInspect, onVoices, onFocusNew }: {
  segment: AudioSegment; index: number; siblings: AudioSegment[]; snapshot: AudioProjectSnapshot; selected: boolean; busy: boolean;
  action: AudioAction; onSelect: () => void; onInspect: () => void; onVoices: () => void; onFocusNew: (id: string, atEnd?: boolean) => void;
}) {
