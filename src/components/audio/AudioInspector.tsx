...
import { AudioPlayer, Field, GenerationJobs, SavedText, timeLabel } from "@/components/audioMusic/shared";
...
export function AudioInspector({ projectId, chapterId, snapshot, segment, takeId, tab, onTabChange, onTake, onClip, onAddSources, action, busy, onVoices }: {
  ...
  return <div className="as-inspector-content">
    ...
    {tab === "exports" ? <><AudioExports snapshot={snapshot} />{!snapshot.exports.length && <div className="as-inspector-empty"><p>还没有导出成品</p><small>将声音放入时间线后，从底部导出 WAV。</small></div>}</> : <>
      ...
      {tab === "voice" && segment && <SpeechControls key={`${segment.id}:${speaker?.id ?? ""}`} segment={segment} speaker={speaker} speakers={snapshot.speakers} onVoices={onVoices} />}
