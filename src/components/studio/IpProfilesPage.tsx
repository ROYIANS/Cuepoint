...
export function IpProfilePage({ ipId }: { ipId: string }) {
  ...
  return <main className="ip-page ip-detail-page">
    ...
    {tab === "profile" && <><div className="ip-detail-fields">{IP_TEXT_FIELDS.map(field => <section key={field.key}><h2>{field.label}</h2><p className={!profile[field.key] ? "is-empty" : undefined}>{profile[field.key] || "尚未填写"}</p></section>)}</div><p className="ip-page-footnote">关联项目的聊天会读取这份 IP 档案的精简摘要，完整设定按需查阅。生成仍以你确认的提示词和参数为准。</p><DerivativeEntries /></>}
