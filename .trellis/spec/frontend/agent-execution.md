...
## 1. Scope / Trigger
Read when changing Agent messages, execution lifecycle, retry, tool-loop integration or startup recovery. IndexedDB v8 adds studio-global agents and agentRuns. These records and credentials are excluded from project ZIPs.
...
- `AgentRunStatus`: running / completed / failed / cancelled / interrupted.
...
- Explicit retry creates a new run/assistant message and references its predecessor, while reusing the original user message and frozen request snapshot. Only the latest unsuccessful attempt without later user turns can retry. Connector ID/provider/base URL and model must match; current credentials may rotate. No startup network resubmission.
