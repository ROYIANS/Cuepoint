...
- The two foundation tools remain: workspace_overview reads bounded project titles/counts; update_run_plan changes the current run checklist atomically with its result. They are joined by 33 business tools and 5 generation tools across six skill groups; see creative skills for the supported surface. Eligible Task-mode runs additionally receive five current-task bookkeeping tools (task_read/create/update/record_read/record_write) and update_run_plan independent of optional skill switches; see agent-tasks.md.
