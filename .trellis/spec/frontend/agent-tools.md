...
## 1. Scope / Trigger
Read when changing tool protocol, Agent execution steps, approval UI, skills or configuration. This extends [execution foundation](./agent-execution.md); no business CRUD, generation or search adapters are wired yet.

## 2. Signatures
- `AgentPermissionMode`: ask / assist / full.
- `AgentToolDefinition`: name/title/description/parameters/effect/parseArguments/highRisk/execute.
...
- Ask gates business write/network tools; assist gates high-risk tools; full permits available validated tools without human approval. Risk/effect come from registry code, not model arguments. Effect/risk must match the approved durable call snapshot before execution.
- Two builtins ship: workspace_overview reads bounded project titles/counts; update_run_plan changes the current run checklist. Plan change and its successful result share one database transaction. Neither writes production assets or uses internet.
...
- Max 8 model requests per run, max 16 calls per model response, 32,768 argument characters per call, 65,536 result characters. A fresh resume does not reset the model budget.
...
- Interrupted running side effects become unknown, block automatic continuation and initial-prompt retry. Users may end the run after checking results. A tool run cannot be restarted from the original prompt; use saved continuation context.
...
| Full | No manual approval, still validation/ownership checks |
...
| Model budget exhausted | Explicit failure; resuming does not reset budget |
...
## 6. Tests Required
`agentTools.test.ts`: all permission branches with controlled definitions, immutable decision ownership, invalid arguments, original settings snapshot, bounded loop, duplicate call IDs, approve/reject continuation, uncertain effects, deletion, Stop and save failure, atomic plan result.
