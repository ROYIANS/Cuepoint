...
## Data contracts
- Reuse stable-ID acceptance expectations from task orchestration; old tasks normalize to no expectations. Separate acceptance findings from plan-item execution status. Recheck schema after child1.
...
- Bounded source assembly retains goals, corrections, unresolved outcomes and plan state. Large histories use explicit coverage and staged read-only summarization; omitted history is disclosed. No claims of exhaustive verification from truncated input.
