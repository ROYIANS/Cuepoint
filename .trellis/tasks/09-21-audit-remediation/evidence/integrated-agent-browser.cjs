...
(async()=>{const browser=await chromium.launch({executablePath:'/Applications/Microsoft Edge.app/Contents/MacOS/Microsoft Edge',headless:true});const context=await browser.newContext({viewport:{width:1440,height:1000}}),page=await context.newPage();const errors=[],requests=[];page.on('pageerror',e=>errors.push(e.message));try{
 ...
 await page.screenshot({path:'.trellis/tasks/09-21-audit-remediation/evidence/integration-desktop.png',animations:'disabled'});
 await page.setViewportSize({width:390,height:844});assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));await page.screenshot({path:'.trellis/tasks/09-21-audit-remediation/evidence/integration-mobile.png',animations:'disabled'});
 ...
 await page.screenshot({path:'.trellis/tasks/09-21-audit-remediation/evidence/integration-combined-mobile.png',animations:'disabled'});
 await page.getByText('查看项目图片',{exact:true}).click();
 await page.getByRole('region',{name:'联网调研来源'}).last().scrollIntoViewIfNeeded();
 await page.screenshot({path:'.trellis/tasks/09-21-audit-remediation/evidence/integration-combined-web-mobile.png',animations:'disabled'});
